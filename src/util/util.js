...
(function (global) {
	...
	var Utils = {
		...
		isPlainObject(obj) {
			...
			if (
				...
				!Object.constructor.prototype.hasOwnProperty.call(
					obj,
					'isPrototypeOf'
				)
			) {
					...
					for (name in options) {
						...
						if (
							...
							(this.isPlainObject(copy) ||
								(copyIsArray = this.typeOf(copy) === 'Array'))
						) {
							if (copyIsArray) {
								copyIsArray = false;
								clone =
									src && this.typeOf(src) === 'Array'
										? src
										: [];
							} else {
								clone =
									src && this.isPlainObject(src) ? src : {};
							}
	...
	var dateUtil = {
		...
		dateFormat(dDate, sFmt) {
			...
			if (/(y+)/.test(sFmt))
				sFmt = sFmt.replace(
					RegExp.$1,
					(dDate.getFullYear() + '').substr(4 - RegExp.$1.length)
				);
			for (var k in o)
		...
		dateOffset: function (
			...
			var _this = this;
			var t =
				_this.parseDate(dTargetDate).getTime() -
		 ...
		 */
		timeAddOrSub: function (
			dDate,
			dDate2 = { y: 0, M: 0, d: 0, h: 0, m: 0, s: 0 },
			add = '+'
		) {
			...
			var result = isAdd
				? dateTimeStamp + dateTimeStampCount
				: dateTimeStamp - dateTimeStampCount;
	...
	var gidUtil = {
		...
		md5: function (sValue, sKey, bRaw) {
			return (
				(window.md5 && window.md5(sValue, sKey, bRaw).toUpperCase()) ||
				null
			);
		},
		encode: function (sValue) {
			return btoa(
				encodeURIComponent(sValue).replace(
					/%([0-9A-F]{2})/g,
		...
		gid: function () {
			...
			return (
				_this.md5(new Date() + ',' + _this.random(1000, 9999)) ||
				_this.gsid(32)
			);
		},
