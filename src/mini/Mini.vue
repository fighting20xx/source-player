<template>
	<div class="mini">
		<div class="top">
			<div class="left">
				<span class="title">
					<span v-if="m3u8Arr.length > 1"
						>『第 {{ video.index + 1 }} 集』</span
					>{{ name }}
				</span>
				<span
					class="zy-svg"
					@click="prevEvent"
					v-show="video.index > 0"
				>
					<svg
						role="img"
						...
						height="24"
						viewBox="0 0 24 24"
						aria-labelledby="backIconTitle"
					>
						<title id="backIconTitle">上一集</title>
						<path
							d="M14 14.74L21 19V5l-7 4.26V5L2 12l12 7v-4.26z"
						></path>
					</svg>
				</span>
				<span
					class="zy-svg"
					@click="nextEvent"
					v-show="video.index < m3u8Arr.length - 1"
				>
					<svg
						role="img"
						xmlns="http://www.w3.org/2000/svg"
				...
				<span class="opacity" v-show="opacity !== 100"
					>透明度: {{ opacity }}</span
				>
				<span class="rate" v-show="rate !== 1"
					>播放速率: {{ rate }}</span
				>
				<span class="progress" v-show="progress > 0"
					>播放进度: {{ progress }}%</span
				>
			</div>
			<div class="right">
