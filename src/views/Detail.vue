			...
			<div class="detail-body zy-scroll" v-show="!loading">
				<div class="info">
					...
					<div class="info-right">
						...
						<div class="director" v-show="info.director">
							导演: {{ info.director }}
						</div>
						<div class="actor" v-show="info.actor">
							主演: {{ info.actor }}
						</div>
						<div class="type" v-show="info.type">
							类型: {{ info.type }}
						</div>
						<div class="area" v-show="info.area">
							地区: {{ info.area }}
						</div>
						<div class="lang" v-show="info.lang">
							语言: {{ info.lang }}
						</div>
						<div class="year" v-show="info.year">
							上映: {{ info.year }}
						</div>
						<div class="last" v-show="info.last">
							更新: {{ info.last }}
						</div>
						<div class="note" v-show="info.note">
							备注: {{ info.note }}
						</div>
						<div class="rate" v-show="info.rate">
							豆瓣评分: {{ info.rate }}
						</div>
					</div>
				</div>
	...
	methods: {
		...
		doubanLinkEvent() {
			...
			axios.get(doubanSearchLink).then((res) => {
				...
				if (
					name.replace(/\s/g, "") ===
					nameInDouban.text().replace(/\s/g, "")
				) {
					link = nameInDouban.attr("href");
				} else {
					nameInDouban = $($("div.result")[1])
						.find("div>div>h3>a")
						.first();
					if (
						name.replace(/\s/g, "") ===
