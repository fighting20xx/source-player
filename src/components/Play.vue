<template>
	<div class="play">
		<div class="box">
			<div class="title">
				<span v-if="this.right.list.length > 1"
					>『第 {{ video.info.index + 1 }} 集』</span
				>{{ name }}
				<span
					v-if="video.key"
					class="right"
			...
			<div class="more">
				<span class="zy-svg" @click="nextEvent" v-show="showNext">
					...
					>
						...
						<path
							d="M10 14.74L3 19V5l7 4.26V5l12 7-12 7v-4.26z"
						></path>
					</svg>
				</span>
				<span
				...
				>
					...
					>
						...
						<path
							d="M5 2h14a3 3 0 0 1 3 3v17H2V5a3 3 0 0 1 3-3z"
						></path>
					</svg>
				</span>
				<span
				...
				<span
					...
					>上次播放到【{{ right.history[0].site }}】{{
						right.history[0].name
					}}
					第{{ right.history[0].index + 1 }}集</span
				>
			</div>
		</div>
	...
	watch: {
		...
		name() {
			...
			if (this.right.list.length > 1) {
				span.innerText = `『第 ${this.video.info.index + 1} 集』${
					this.name
				}`;
			} else {
	...
	methods: {
		...
		issueEvent() {
			const info = {
				...
				m3u8List:
					VIDEO_DETAIL_CACHE[
						this.video.key + "@" + this.video.info.id
					] || [],
		...
		playWithExternalPalyerEvent() {
			this.fetchM3u8List().then(m3u8Arr => {
				...
				if (!externalPlayer) {
					...
					// 在线播放该视频
					var link =
						"https://www.m3u8play.com/?play=" +
						m3u8Arr[this.video.info.index];
					const open = require("open");
					open(link);
				} else {
					var m3uFile = this.generateM3uFile(
						this.video.info.name,
						m3u8Arr,
						this.video.info.index
					);
		...
		toggleList() {
			if (this.state.showList) {
				document.querySelector("xg-btn-showlist ul").style.display =
					"none";
		...
		toggleHistory() {
			if (this.state.showHistory) {
				document.querySelector("xg-btn-showhistory ul").style.display =
					"none";
		...
		refreshHistory() {
			...
			if (!ul) {
				...
				ul.addEventListener("click", (ev) => {
					...
					const target = ev.target || ev.srcElement; // target表示在事件冒泡中触发事件的源元素，在IE中是srcElement
					if (target.nodeName.toLowerCase() === "li") {
						this.historyItemEvent(
							this.right.history[parseInt(target.dataset.index)]
						);
					}
				});
			}
			...
			if (this.right.history.length === 0) {
				...
				for (
					let index = 0;
					index < this.right.history.length;
					index++
				) {
					const item = this.right.history[index];
					const text = `【${item.site}】${item.name} 第${
						item.index + 1
					}集`;
		...
		bindEvent() {
			...
			this.xg.on("exitFullscreen", () => {
				document.querySelector(".xg-view-videoTitle").style.display =
					"none";
			});
		},
	},
	...
	mounted() {
		...
		Player.install("showHistory", function () {
			addPlayerBtn.bind(
				this,
				"showHistory",
				'<svg t="1595866015473" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3282" style="width: 22px;height: 22px;margin-top: 9px;margin-left: 6px;" xmlns:xlink="http://www.w3.org/1999/xlink"><path d="M512 0a512 512 0 1 0 512 512A512 512 0 0 0 512 0z m0 910.222222a398.222222 398.222222 0 1 1 398.222222-398.222222 398.222222 398.222222 0 0 1-398.222222 398.222222z" p-id="3283" fill="#ffffff"></path><path d="M568.888889 227.555556h-113.777778v341.333333h227.555556v-113.777778h-113.777778V227.555556z" p-id="3284" fill="#ffffff"></path></svg>',
				{ title: "播放历史" }
			)();
		});
		const that = this;
