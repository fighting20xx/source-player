			...
			<div class="zy-table">
				...
				<div class="tBody zy-scroll">
					<ul>
						...
						>
							<span class="name" @click.stop="detailEvent(i)">{{
								i.name
							}}</span>
							<span class="site">{{ getSiteName(i.site) }}</span>
							<span class="note">第{{ i.index + 1 }}集</span>
							<span class="operate">
								<span class="btn" @click.stop="playEvent(i)"
									>播放</span
								>
								<span class="btn" @click.stop="shareEvent(i)"
									>分享</span
								>
		...
		playEvent(e) {
			history.find({ site: e.site, ids: e.ids }).then(res => {
				if (res) {
					this.video = {
						key: res.site,
						info: { id: res.ids, name: res.name, index: res.index },
					};
				} else {
					this.video = {
						key: e.site,
						info: { id: e.ids, name: e.name, index: 0 },
					};
				}
			});
			this.view = "Play";
