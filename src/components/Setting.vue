		...
		<div class="setting-box zy-scroll">
			...
			<div class="info">
				...
				<a
					@click="
						linkOpen(
							'https://github.com/Hunlongyu/ZY-Player/issues'
						)
					"
					>v{{ pkg.version }} 反馈</a
				>
			...
			<div class="view">
				...
				<div class="view-box">
					<div class="zy-select" @mouseleave="show.view = false">
						<div class="vs-placeholder" @click="show.view = true">
							默认视图
						</div>
			...
			<div class="search">
				...
				<div class="zy-checkbox">
					<input
						type="checkbox"
						v-model="setting.searchAllSites"
						@change="updateSearchOption($event)"
					/>
					搜索所有资源
				</div>
			</div>
			...
			<div class="clearDB">
				...
				<span class="clearTips"
					>如果新安装用户, 无法显示资源, 请点击软件重置. 如非必要,
					切勿点击. 会清空用户数据, 恢复默认设置. 点击即软件重置,
					并关闭软件.</span
				>
			</div>
			<div class="Tips">
...
export default {
	...
	methods: {
				...
				.then((result) => {
					if (!result.canceled) {
						...
						this.$message.success(
							"设定第三方播放器路径为：" + result.filePaths[0]
						);
		...
		updatePlayerPath() {
			this.$message.success(
				"设定第三方播放器路径为：" + this.externalPlayer
			);
