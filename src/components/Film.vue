	...
	<div class="film">
		<div class="header">
			<div class="zy-select" @mouseleave="show.site = false">
				<div class="vs-placeholder" @click="show.site = true">
					{{ site.name }}
				</div>
			...
			<div
				...
				v-show="show.class"
			>
				<div class="vs-placeholder" @click="show.classList = true">
					{{ type.name }}
				</div>
			...
			<div class="zy-select" @mouseleave="show.search = false">
				<div class="vs-input" @click="show.search = true">
					<input
		...
		playEvent(site, e) {
			history.find({ site: site.key, ids: e.id }).then(res => {
				if (res) {
					...
					this.video = {
						key: site.key,
						info: { id: e.id, name: e.name, index: 0, site: site },
					};
				}
			});
			this.view = "Play";
